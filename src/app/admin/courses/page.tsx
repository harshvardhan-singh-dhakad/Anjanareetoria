...
export default function AdminCoursesPage() {
  ...
  const handleThumbnailUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setMsg({ type: 'success', text: 'Course thumbnail uploaded to persistent CMS media storage.' });
    } catch (err: any) {
      setMsg({ type: 'error', text: err.message || 'Thumbnail upload failed.' });
    } finally {
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3.5 py-2.5 rounded-xl border border-slate-300 text-xs focus:outline-none focus:border-[#1346af]"
                    />
                    <label className="cursor-pointer inline-flex items-center gap-1.5 px-3 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-semibold">
                      <UploadCloud size={14} />
                  ...
                  {thumbnail && (
                    <div className="mt-2 w-28 h-16 rounded-lg overflow-hidden border border-slate-200 bg-slate-50 relative">
                      <Image src={thumbnail} alt="Course thumbnail preview" fill className="object-cover" />
                    </div>
                  )}
                </div>
